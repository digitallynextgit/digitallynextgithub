...
// Color scheme for nuggets - only red and blue
const nuggetColors = [
  "#ff4255", // Red
  "#0ec9c5", // Blue
];
...
const NuggetCard: React.FC<NuggetCardProps> = ({ item }) => {
  ...
  // Get color based on nugget ID (alternating between red and blue)
    ...
    <div className="relative flex flex-col h-full group">
      ...
      <div className="relative z-10 w-full">
        <Carousel className="w-full h-full">
          <CarouselContent>
            {item.images && item.images.map((imageNumber: number) => {
  ...
  return (
    <main className="min-h-screen bg-white py-[10vh] px-[4vw]">
      ...
      <div className="text-center lg:my-[9vh] my-[10vw]">
        <h1 className="text-[11vw] md:text-[4vw] font-bold text-blue">
          {nuggetsData.title}
        </h1>
      </div>
        ...
        <div className="grid grid-cols-1 gap-[4vh]">
          {rows.map((row, rowIndex) => (
            <div key={rowIndex} className="flex gap-[2vw] justify-start">
              ...
              {/* Add empty placeholder divs to maintain grid structure */}
